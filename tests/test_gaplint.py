...
# @pytest.mark.parametrize(
#     "fname,expected",
#     [("tests/input/test1.g", set())],
# )
# def test_column_number_diagnostics(fname, expected):
#     with pytest.raises(SystemExit) as e:
#         run_gaplint(files=[fname], ranges=True)
#     with open(fname, "r", encoding="utf-8") as ffile:
#         lines = ffile.read()
#     lines = lines.split("\n")
#     for diagnostic in gaplint._DIAGNOSTICS:
#         assert 0 <= diagnostic.line < len(lines)
#         if diagnostic.column is not None:
#             assert 0 <= diagnostic.column <= len(lines[diagnostic.line])
#         if diagnostic.column_end is not None:
#             assert diagnostic.line_end is not None
#             assert 0 <= diagnostic.column_end <= len(lines[diagnostic.line_end])
#     assert expected == set(gaplint._DIAGNOSTICS)
#
#
# def test_all_diagnostics(fname, expected):
#     pass
